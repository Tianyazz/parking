...
export default class LicensePlate extends base {
  ...
  */
  static async historyCars () {
    ...
    let data = await this.post(`${this.baseUrl}/park/Content/recent`, param)
    return data
  }
  /*
  * 查询牌照费用
  ...
  */
  static async InquiryFee (param) {
    ...
    let fee = await this.post(`${this.baseUrl}/park/Order/compute`, params)
    return fee
  }
  /*
  * 获取微信支付jsconfig
  */
  static async getPayJsConfig (param) {
    ...
    let config = await this.post(`${this.baseUrl}/park/Pay/PayJs`, params)
    return config
  }
}